...
class ProductoController extends Controller
{
    ...
    {
        $jerarquias = Jerarquia::where('estado_id',1)->get();
        return view('dyls.punto-venta.producto.lista', get_defined_vars());
    }
    ...
    public function listar()
    {
        ...
        return DataTables::of($data)
            ->addColumn('jerarquia', function ($data) {
                return $data->jerarquia->nombre;
            })
            ...
            ->addColumn('accion', function ($data) {
                ...
                '<div class="flex align-items-center list-user-action" >
                    <a href="#" class="btn btn-default btn-icon btn-sm editar" data-id="' . $data->id . '" >
    ...
    {
        try {
            $data = Producto::firstOrNew(['id' => $request->id]);
            // $data->imagen   = $request->imagen;
            $data->descripcion   = $request->descripcion;
            $data->stock   = $request->stock;
            $data->precio   = $request->precio;
            $data->jerarquia_id   = $request->jerarquia_id;
            $data->estado_id   = 1;
            $data->save();

            $data = Producto::find($data->id);
            $data->codigo   = 'PD-'.$this->zero_fill($data->id, 4) ;
            $data->save();
